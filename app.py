...
def create_sidebar() -> Optional[WorkflowConfig]:
    ...
    with st.sidebar.expander("Edit Source Tiers"):
        ...
        secondary_sources_text = st.text_area(
            ...
            value="\n".join(DEFAULT_SECONDARY_SOURCES),
            height=220,
...
async def run_workflow_async(config, event_queue, cancel_event):
    ...
    try:
        ...
        class StreamlitMultiAgentWorkflow(MultiAgentWorkflow):
            """Custom workflow class that captures events and sends them to the UI queue."""

            def __init__(self, config, event_queue, cancel_event):
            ...
            async def run(self):
                try:
                    ...
                    async for event in handler.stream_events():
                        ...
                        elif hasattr(event, "tool_name") and hasattr(
                            event, "tool_output"
                        ):
                            self.event_queue.put(
                                {
                ...
                # Catch all exceptions during workflow execution to ensure proper error reporting
        ...
        workflow = StreamlitMultiAgentWorkflow(config, event_queue, cancel_event)
        await workflow.run()
    ...
    with col1:
        ...
        with col_clear:
            clear_button = st.button(
                "🗑️ Clear Results", help="Clear all workflow results and logs"
            )
    ...
    with col2:
        ...
        if st.session_state.workflow_running:
            with st.spinner("Workflow is running. Please wait..."):
                ...
                st.markdown(
                    "⏱️ **Started:** " + datetime.datetime.now().strftime("%H:%M:%S")
                )
        else:
    ...
    if st.session_state.workflow_running and hasattr(st.session_state, "event_queue"):
        ...
        if hasattr(st.session_state, "workflow_thread"):
            if not st.session_state.workflow_thread.is_alive():
                was_cancelled = bool(
                    hasattr(st.session_state, "cancel_event")
                    and st.session_state.cancel_event.is_set()
                )
                st.session_state.workflow_running = False
                st.session_state.workflow_stop_requested = False
                if was_cancelled:
